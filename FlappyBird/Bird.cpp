...
namespace Sonar
{
	Bird::Bird(GameDataRef data) : _data(data)
	{
		...
		sf::Vector2f origin = sf::Vector2f(_birdSprite.getGlobalBounds().width / 2.f, _birdSprite.getGlobalBounds().height / 2.f);
	...
	void Sonar::Bird::SetScale(float x, float y)
	{
		_birdSprite.setScale(x, y);
	}
